	...
	class SemantickiAnalizator : SemantickiAnalizatorBase
	{
		...
		public void OtvorenBlok(string nImePotprograma)
		{
			...
			if (opisnik!=null)
			{
				...
				foreach(Polje parametar in opisnik.Parametri)
				{
		...
		public void ProvjeriVarijabluIliFunkciju(string nImeIdentifikatora)
		{
			Polje tmp;
			int razina;
			TablicaZnakova.PronadjiVarijablu(nImeIdentifikatora, out tmp, out razina);
			...
			KonstantniIdentifikator konst;
			TablicaZnakova.PronadjiKonstantniIdentifikator(nImeIdentifikatora, out konst);
			if (konst!=null)
			{
				return;
			}
			...
			PrijaviGresku("Ocekivana varijabla ili funkcija.");
		}
			...
			UInt32 inAdresa, TipPodatka inTipIzraza, UInt32 inVrijednostIzraza, bool inSveKonstantno)
		{
			...
			if (inTipIzraza == null) {
				PrijaviGresku("Tip izraza je nedefiniran.");
				inTipIzraza = inTipVarijable;
			}
		...
		public void PozoviFunkciju(string nImePotprograma, out TipPodatka iTipKojiSeVraca)
		{
			...
			else
			{
				...
				{
					PrijaviGresku("Ocekivano ime funkcije.");
				}
				else
				{
		...
		{	
			if (nTipDesnogOperanda == null) {
				PrijaviGresku("Tip desnog operanda nije definiran!");
				nTipDesnogOperanda = nTipLijevogOperanda;
			}
			if (
				(nTipDesnogOperanda.Tip != nTipLijevogOperanda.Tip) && 
				(
				((nTipDesnogOperanda.Tip!=UgradjeniTipoviEnum.Integer) && (nTipDesnogOperanda.Tip!=UgradjeniTipoviEnum.Real) && (nTipDesnogOperanda.Tip!=UgradjeniTipoviEnum.Interval)) ||
				((nTipLijevogOperanda.Tip!=UgradjeniTipoviEnum.Integer) && (nTipLijevogOperanda.Tip != UgradjeniTipoviEnum.Real) && (nTipLijevogOperanda.Tip!=UgradjeniTipoviEnum.Interval))
				)
				)
			{
					...
					case BinarniOperatoriEnum.Plus:
					{
						...
						else
						{
							if (nTipLijevogOperanda.Tip==UgradjeniTipoviEnum.Real)
							{
								...
								if (!iSveKonstantnoEval) 
								{
									if (nLijevoKonstantno && !nDesnoKonstantno) GeneratorMedjukoda.Generiraj(DvoadresneNaredbeEnum.Lit,nVrijedostLijeve);
									if (nDesnoKonstantno && !nLijevoKonstantno) GeneratorMedjukoda.Generiraj(DvoadresneNaredbeEnum.Lit,nVrijednostDesne);
									GeneratorMedjukoda.Generiraj(JednoadresneNaredbeEnum.Add_r);

								}
								else iUkupnaVrijednost = KonverterPodataka.FloatToBitRepresentation(f);
							}
							else 
							{
								int i;
					...
					case BinarniOperatoriEnum.Minus:
					{
						...
						else
						{
							...
							{	
								...
								if(!iSveKonstantnoEval)
								{
									if (nLijevoKonstantno && !nDesnoKonstantno) 
									{

										GeneratorMedjukoda.Generiraj(DvoadresneNaredbeEnum.Lit,nVrijedostLijeve);
									}
									if (nDesnoKonstantno && !nLijevoKonstantno) GeneratorMedjukoda.Generiraj(DvoadresneNaredbeEnum.Lit,nVrijednostDesne);									
									GeneratorMedjukoda.Generiraj(JednoadresneNaredbeEnum.Sub_r);
									if (nLijevoKonstantno && !nDesnoKonstantno) GeneratorMedjukoda.Generiraj(JednoadresneNaredbeEnum.Neg_r);
								}
								else iUkupnaVrijednost = KonverterPodataka.FloatToBitRepresentation(f);
							} 
							else 
							{
								...
								if(!iSveKonstantnoEval)
								{
									...
									if (nLijevoKonstantno && !nDesnoKonstantno) GeneratorMedjukoda.Generiraj(JednoadresneNaredbeEnum.Neg_i);
								}
								else iUkupnaVrijednost = KonverterPodataka.SignedIntToBitRepresentation(i);
							}
						}
						break;
					}
						...
						else
						{
							if (nTipLijevogOperanda.Tip == UgradjeniTipoviEnum.Real) 
							{
								...
								if(!iSveKonstantnoEval) 
								{
									...
									GeneratorMedjukoda.Generiraj(JednoadresneNaredbeEnum.Mul_r);
								}
								else iUkupnaVrijednost = KonverterPodataka.FloatToBitRepresentation(f);
							} 
							else 
					...
					case BinarniOperatoriEnum.Dijeljeno:
					{
						...
						else							
						{
							...
							else
							{
								if (nLijevoKonstantno && !nDesnoKonstantno) 									
								{
									GeneratorMedjukoda.Generiraj(TroadresneNaredbeEnum.Sto,KonverterPodataka.SignedIntToBitRepresentation(0),KonverterPodataka.SignedIntToBitRepresentation(TrenutniKontekst.VelicinaOpisnikaNaStogu + TrenutniKontekst.VelicinaStogaZaParametre));
									GeneratorMedjukoda.Generiraj(DvoadresneNaredbeEnum.Lit,nVrijedostLijeve);
									GeneratorMedjukoda.Generiraj(TroadresneNaredbeEnum.Lod,KonverterPodataka.SignedIntToBitRepresentation(0),KonverterPodataka.SignedIntToBitRepresentation( TrenutniKontekst.VelicinaOpisnikaNaStogu + TrenutniKontekst.VelicinaStogaZaParametre));
								}
								if (nDesnoKonstantno && !nLijevoKonstantno) GeneratorMedjukoda.Generiraj(DvoadresneNaredbeEnum.Lit,nVrijednostDesne);																		
								GeneratorMedjukoda.Generiraj(JednoadresneNaredbeEnum.Div_r);
						...
						else
						{
							...
							if(!iSveKonstantnoEval)
							{
								if (nLijevoKonstantno && !nDesnoKonstantno) 									
									GeneratorMedjukoda.Generiraj(DvoadresneNaredbeEnum.Lit,nVrijedostLijeve);
								if (nDesnoKonstantno && !nLijevoKonstantno) GeneratorMedjukoda.Generiraj(DvoadresneNaredbeEnum.Lit,nVrijednostDesne);																		
								GeneratorMedjukoda.Generiraj(JednoadresneNaredbeEnum.And);
							}
							else iUkupnaVrijednost = KonverterPodataka.BoolToBitRepresentation(lijeva && desna);
							iEvaluiraniTip = KonstruktoriTipova.Boolean();
						}
						break;
					}
						...
						else
						{
							if (nTipLijevogOperanda.Tip == UgradjeniTipoviEnum.Real)
							{
								...
								if(!iSveKonstantnoEval)
								{
									...
									GeneratorMedjukoda.Generiraj(JednoadresneNaredbeEnum.Eql_r);
								}
								else iUkupnaVrijednost = KonverterPodataka.BoolToBitRepresentation(lijeva == desna);
							}
							else if(nTipLijevogOperanda.Tip == UgradjeniTipoviEnum.Boolean)
							{
								...
								else iUkupnaVrijednost = KonverterPodataka.BoolToBitRepresentation(KonverterPodataka.BitRepresentationToBool( nVrijedostLijeve) == KonverterPodataka.BitRepresentationToBool( nVrijednostDesne));								
							}
						...
						else
						{
							if (nTipLijevogOperanda.Tip == UgradjeniTipoviEnum.Real) 
							{
								...
								if(!iSveKonstantnoEval)
								{
									...
									if(nLijevoKonstantno && ! nDesnoKonstantno) 
									{
										GeneratorMedjukoda.Generiraj(JednoadresneNaredbeEnum.Gtr_r);
										
									}
									else GeneratorMedjukoda.Generiraj(JednoadresneNaredbeEnum.Leq_r);
								}
								else iUkupnaVrijednost = KonverterPodataka.BoolToBitRepresentation(lijeva <= desna);
							} 
							else if (nTipLijevogOperanda.Tip == UgradjeniTipoviEnum.Integer)
							{
								int lijeva = KonverterPodataka.BitRepresentationToSignedInt(nVrijedostLijeve);
								int desna = KonverterPodataka.BitRepresentationToSignedInt(nVrijednostDesne);
								...
								if(!iSveKonstantnoEval)
								{
									...
									else GeneratorMedjukoda.Generiraj(JednoadresneNaredbeEnum.Leq_i);
								}
								else iUkupnaVrijednost = KonverterPodataka.BoolToBitRepresentation(lijeva <= desna);
							}
							iEvaluiraniTip = KonstruktoriTipova.Boolean();
						...
						else
						{
							if (nTipLijevogOperanda.Tip == UgradjeniTipoviEnum.Real) 
							{
								...
								if(!iSveKonstantnoEval)
								{
									...
									if(nLijevoKonstantno && ! nDesnoKonstantno) 
									{
										GeneratorMedjukoda.Generiraj(JednoadresneNaredbeEnum.Geq_r);
										
									}
									else GeneratorMedjukoda.Generiraj(JednoadresneNaredbeEnum.Lss_r);
								}
								else iUkupnaVrijednost = KonverterPodataka.BoolToBitRepresentation(lijeva < desna);
							} 
							else 
							{
								...
								if(!iSveKonstantnoEval)
								{
									...
									if(nLijevoKonstantno && ! nDesnoKonstantno) 
									{
										GeneratorMedjukoda.Generiraj(JednoadresneNaredbeEnum.Geq_i);
										
									}
									else GeneratorMedjukoda.Generiraj(JednoadresneNaredbeEnum.Lss_i);
								}
								else iUkupnaVrijednost = KonverterPodataka.BoolToBitRepresentation(lijeva < desna);
							}
							iEvaluiraniTip = KonstruktoriTipova.Boolean();
					...
					case BinarniOperatoriEnum.Mod:
					{
						...
						else
						{
							...
							if(!iSveKonstantnoEval) 
							{
								...
								if (nDesnoKonstantno && !nLijevoKonstantno) GeneratorMedjukoda.Generiraj(DvoadresneNaredbeEnum.Lit,nVrijednostDesne);																		
								GeneratorMedjukoda.Generiraj(JednoadresneNaredbeEnum.Mod);
								
							}
							else iUkupnaVrijednost = KonverterPodataka.SignedIntToBitRepresentation(KonverterPodataka.BitRepresentationToSignedInt( nVrijedostLijeve) % KonverterPodataka.BitRepresentationToSignedInt( nVrijednostDesne));
						}
						break;
					}
						...
						else
						{
							if (nTipLijevogOperanda.Tip == UgradjeniTipoviEnum.Real) 
							{
								...
								if(!iSveKonstantnoEval )
								{
									if (nLijevoKonstantno && !nDesnoKonstantno) GeneratorMedjukoda.Generiraj(DvoadresneNaredbeEnum.Lit,nVrijedostLijeve);
									if(nDesnoKonstantno && ! nLijevoKonstantno) GeneratorMedjukoda.Generiraj(DvoadresneNaredbeEnum.Lit,nVrijednostDesne);
									GeneratorMedjukoda.Generiraj(JednoadresneNaredbeEnum.Neq_r);
								}
								else iUkupnaVrijednost = KonverterPodataka.BoolToBitRepresentation(lijeva != desna);
							} 
							else if (nTipLijevogOperanda.Tip == UgradjeniTipoviEnum.Boolean)
							{
								...
								if (iSveKonstantnoEval) iUkupnaVrijednost = KonverterPodataka.BoolToBitRepresentation(KonverterPodataka.BitRepresentationToBool( nVrijedostLijeve) != KonverterPodataka.BitRepresentationToBool( nVrijednostDesne));
								else GeneratorMedjukoda.Generiraj(JednoadresneNaredbeEnum.Neq_i);
							}
							else 
							{
								int lijeva = KonverterPodataka.BitRepresentationToSignedInt(nVrijedostLijeve);
								...
								if(!iSveKonstantnoEval )
								{
									if (nLijevoKonstantno && !nDesnoKonstantno) GeneratorMedjukoda.Generiraj(DvoadresneNaredbeEnum.Lit,nVrijedostLijeve);
									if(nDesnoKonstantno && ! nLijevoKonstantno) GeneratorMedjukoda.Generiraj(DvoadresneNaredbeEnum.Lit,nVrijednostDesne);									
									GeneratorMedjukoda.Generiraj(JednoadresneNaredbeEnum.Neq_i);
								}
								else iUkupnaVrijednost = KonverterPodataka.BoolToBitRepresentation(lijeva != desna);
					...
					case BinarniOperatoriEnum.Or:
					{
						...
						{								
							...
							if(!iSveKonstantnoEval )
							{
								if (nLijevoKonstantno && !nDesnoKonstantno) GeneratorMedjukoda.Generiraj(DvoadresneNaredbeEnum.Lit,nVrijedostLijeve);
								if(nDesnoKonstantno && ! nLijevoKonstantno) GeneratorMedjukoda.Generiraj(DvoadresneNaredbeEnum.Lit,nVrijednostDesne);									
								GeneratorMedjukoda.Generiraj(JednoadresneNaredbeEnum.Or);
							}
							else iUkupnaVrijednost = KonverterPodataka.BoolToBitRepresentation(lijeva || desna);
						}
						break;
						...
						else
						{
							if (nTipLijevogOperanda.Tip == UgradjeniTipoviEnum.Real) 
							{
								...
								if(!iSveKonstantnoEval)
								{
									...
									if(nLijevoKonstantno && ! nDesnoKonstantno) 
									{
										GeneratorMedjukoda.Generiraj(JednoadresneNaredbeEnum.Lss_r);
										
									}
									else GeneratorMedjukoda.Generiraj(JednoadresneNaredbeEnum.Geq_r);
								}
								else iUkupnaVrijednost = KonverterPodataka.BoolToBitRepresentation(lijeva >= desna);
							} 
							else 
							{
								...
								if(!iSveKonstantnoEval)
								{
									...
									else GeneratorMedjukoda.Generiraj(JednoadresneNaredbeEnum.Geq_i);
								}
								else iUkupnaVrijednost = KonverterPodataka.BoolToBitRepresentation(lijeva >= desna);
							}
							iEvaluiraniTip = KonstruktoriTipova.Boolean();
						...
						else
						{
							if (nTipLijevogOperanda.Tip == UgradjeniTipoviEnum.Real) 
							{
								...
								if(!iSveKonstantnoEval)
								{
									...
									if(nLijevoKonstantno && ! nDesnoKonstantno) 
									{
										GeneratorMedjukoda.Generiraj(JednoadresneNaredbeEnum.Leq_r);
										
									}
									else GeneratorMedjukoda.Generiraj(JednoadresneNaredbeEnum.Gtr_r);
								}
								else iUkupnaVrijednost = KonverterPodataka.BoolToBitRepresentation(lijeva > desna);
							} 
							else 
							{
								...
								if(!iSveKonstantnoEval)
								{
									...
									if(nLijevoKonstantno && ! nDesnoKonstantno) 
									{
										GeneratorMedjukoda.Generiraj(JednoadresneNaredbeEnum.Leq_i);
										
									}
									else GeneratorMedjukoda.Generiraj(JednoadresneNaredbeEnum.Gtr_i);
								}
								else iUkupnaVrijednost = KonverterPodataka.BoolToBitRepresentation(lijeva > desna);
							}
							iEvaluiraniTip = KonstruktoriTipova.Boolean();
					...
					case BinarniOperatoriEnum.Div:
					{
						...
						else
						{
							...
							else
							{
								...
								if (nDesnoKonstantno && !nLijevoKonstantno) GeneratorMedjukoda.Generiraj(DvoadresneNaredbeEnum.Lit,nVrijednostDesne);																		
								GeneratorMedjukoda.Generiraj(JednoadresneNaredbeEnum.Div_i);
			...
			bool nSveKonstantno, int nRedniBrojSubscripta, out int iRedniBrojSubscripta)
		{
			...
			if (nRedniBrojSubscripta==nTipNiza.ListaIndeksa.Count-1)
			{
				//pomnozi sve sa velicinom baznog tipa
				GeneratorMedjukoda.Generiraj(DvoadresneNaredbeEnum.Lit, 
					KonverterPodataka.SignedIntToBitRepresentation(nTipNiza.PodTip.VelicinaUBajtovima));
				GeneratorMedjukoda.Generiraj(JednoadresneNaredbeEnum.Mul_i);
			}
		}
